...
CRYPTO_FOLDER = Path.joinpath(
    Path(__file__).absolute().parents[1], "crypto_images")
...
def initHideAndEncrypt():
    ...
    if file and allowed_file(file.filename):
        ...
        return jsonify({"message": "Allowed image types are png, jpg"}), 400
...
def initShowAndDecrypt():
    ...
    if share1 and share2 and allowed_file(share1.filename) and allowed_file(share1.filename):
        ...
        finalImgName = n_share.compress_shares(
            share1id+".png", share2id+".png")
        decrypteddata = lsb_stegno.lsb_decode(
            "../decrypted_images/"+finalImgName)
        return jsonify({
